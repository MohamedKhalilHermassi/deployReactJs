...
function Events() {
    ...
    const fetchEvents = useCallback(async ()  => {
      ...
      setError(null);
      try {
          const response = await axios.get('https://deploynodejs-2.onrender.com/events');
      ...
      {events.filter(event => event.status !== 'Finished' && event.status !== 'Canceled').map(event => (
        <div key={event._id} className="col-md-12 mb-4">
          <Card className="h-100 shadow flex-md-row">
                  <Card.Img variant="left" src={`https://deploynodejs-2.onrender.com${event.image}`} style={{ width: '30%', objectFit: 'cover' }} />
