...
const SessionEdit = () => {
  ...
  useEffect(() => {
    const fetchSession = async () => {
      try {
        const sessionResponse = await fetch(`https://deploynodejs-2.onrender.com/sessions/${id}`);
        const sessionData = await sessionResponse.json();
        const coursesResponse = await fetch('https://deploynodejs-2.onrender.com/courses');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`https://deploynodejs-2.onrender.com/sessions/${id}`, {
        method: 'PUT',
        headers: {
