...
function BookList() {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await axios.get('https://deploynodejs-2.onrender.com/book/get-books');
  ...
  const handleDeleteBook = async (id) => {
    try {
      await axios.delete(`https://deploynodejs-2.onrender.com/book/delete-book/${id}`);
        ...
        <div>
          ...
          {selectedBook && (
            <div className="mt-3">
              ...
              <iframe
                src={`https://deploynodejs-2.onrender.com/uploads/${selectedBook.filename}`}
                width="100%"
                height="600px"
              ></iframe>
            </div>
