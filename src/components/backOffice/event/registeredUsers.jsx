...
const RegisteredUsers = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        // Fetch all events
        const eventsResponse = await axios.get(`https://deploynodejs-2.onrender.com/events`);
        const events = eventsResponse.data;
    
        // For each event, fetch its users
        let allUsers = [];
        for (const event of events) {
          const usersResponse = await axios.get(`https://deploynodejs-2.onrender.com/events/event/${event._id}/users`);
